...
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{\pathRTS/images/rtos.png}
  \caption{RTOS}
  \label{fig:rtos}
\end{subfigure}%
\begin{subfigure}{.5\textwidth}
  \centering
  \includegraphics[width=\linewidth]{\pathRTS/images/other_os.png}
  \caption{Other OS}
  \label{fig:other_os}
\end{subfigure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/timing_constraints.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/handling_an_interrupt.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/feasible_schedule.png}
    \caption{Handling an Interrupt, \cite{RTOS, p.29}}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/scheduling_window.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/dbf.png}
    \caption{DBF graph}
    \label{fig:dbf}
\end{figure}
...
Jitter refers to the unpredictable variation in the timing of a periodic event, the deviation from the expected or ideal timing of when a task is executed.

\paragraph{Release Jitter} The time variation of a periodic task being released, i.e., made ready to run, relative to its ideal time. For instance, when a task is schedule to run every 10ms might some times start at 9.9ms, 10.1ms or 10.3ms due to system load or scheduling delay.
\paragraph{Execution Jitter} The variation in the execution time of a task from one instance to the next. A task might usually take 2ms but due to cache misses or resource contention it might take 2.5ms.
\paragraph{Completion Jitter} The variation in the time at which a task finishes execution relative to its deadline or expected completion time. For instance a task that should finish by 10 ms might complete at 9.8 ms, 10.2 ms, or 10.5 ms.
\paragraph{Inter-Arrival Jitter} The variation in the time between the arrivals of periodic events, e.g., sensor data, network packets. Could occur if a sensor sending data every 5 ms but sometimes send it at 4.9 ms, 5.0 ms, or 5.1 ms intervals.

\paragraph{Calculating the Worst-Case Response Time}
The worst-case response time ($R_i$) for task $\tau_i$ for FP-scheduling on a single processor would then be:
\begin{equation*}
  R_i = \mathpzc{J}_i + \mathpzc{w}_i,
\end{equation*}
where $\mathpzc{J}$ is the jitter, and $\mathpzc{w}$
\begin{equation*}
  \mathpzc{w}_i = C_i + \sum_{\tau_j \in hp(\tau_i)} \left\lceil\frac{\mathpzc{w}_i + \mathpzc{J}_j}{T_j}\right\rceil C_j, \\
\end{equation*}
$C$ is worst-case execution time, the sum calculate $\tau_j$ is a higher priority task. The sum calculates the maximum number of times the higher priority task ($\tau_j$) can preempt task $\tau_i$ during the interval $\mathpzc{w}_i+\mathpzc{J}_j$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/mixed-criticality_HI.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/mixed-criticality_LO.png}
    \caption{Timing Constraints, \cite{08A-advanced-workload-model, p.11}}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/hierarchy_of_models.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/bip_deadlock.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/hlp_example.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/multiprocessor_scheduling_of_sequantial_tasks.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/lakshmanan's_algorithm.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/breadth-first_partiononing_algorithms.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{\pathRTS/images/can_arbitration_mechanism.png}
