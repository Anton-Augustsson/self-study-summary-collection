...
An ISA is the fundamental contract between the hardware mostly in terms of the CPU, and the software. The ISA contains a set of instruction describing fundamental operations, like adding two registers or load words from memory into a CPU register. There are several types of ISA used, but they can be divided into reduced instruction set computing (RISC) and complex instruction set computing (CISC). The x86 ISA is a CISC and was created by Intel but now maintained by the industry, e.g., AMD, Microsoft, Intel, Google, and Orical in \textbf{x86 Ecosystem Advisory Group}. On the other hand, there are several RISC ISAs, e.g., MIPS, ARM, and RISC-V.
...
\begin{itemize}
\item  Divide by zero.
\item  Misaligned memory access, occurs when a program tries to access memory that is perfectly divisible by the data size, e.g., a 4-byte \textit{int} at address \textit{0x10001} instead of having a 4-byte \textit{int} as address \textit{0x10004}.
\item  Page fault, occurs when a program tries to access a virtual memory page not currently in RAM.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Synchronous: always occurs at the same place in a program execution.
    \item Asynchronous: instead caused by external devices that can happen at different times in a programs' execution.
  \end{itemize}
  \item User requested vs. coerced
  \begin{itemize}
    \item Coerced: hardware events that the user can't control
    \item User requested: exceptions requested from the user.
  \end{itemize}
  \item User maskable vs. nonmaskable
  \begin{itemize}
    \item Markable if the user can disable the exception/interrupt.
  \end{itemize}
  \item Within vs. between instructions
  \begin{itemize}
    \item If the event prevents the current instruction from completing, or causes an interrupt after it.
  \end{itemize}
  \item Resume vs. terminate
  \begin{itemize}
    \item If the OS or program can handle the event or if the program must be terminated.
  \end{itemize}
\end{itemize}
...
\section{Cache}
% TODO: cache Pseudo-Locking
% TODO: lockfree data structures https://github.com/DNedic/lockfree
...
\section{Modern High Performance CPUs}
% https://www.youtube.com/watch?v=jC_z1vL1OCI
% Flow from instructions to actual execution.
% Micro operations
% Not just ALU, there are also specialised executions.
