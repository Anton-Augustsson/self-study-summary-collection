...
\begin{figure}[H]
  \center
  \begin{tabular}{ | >{\centering\arraybackslash}m{3cm} | m{3cm} | >{\centering\arraybackslash}m{4cm} | m{3cm} | } 
  \hline
    \tikz\pic at (0,0) {dcv={control mechanism=pedal, directional valve=two port two position normaly closed, scale=0.5}}; & Pedal controlled, 2 port, 2 position, normally closed &
  \tikz\pic at (0,0) {dcv={control mechanism=pedal, directional valve=four port three position closed centre, scale=0.5}}; & Solenoid controlled, 4 port, 3 position, closed centre \\ 
  \hline
    \tikz\pic at (0,0) {dcv={control mechanism=lever, directional valve=two port two position normaly open, scale=0.5}}; & Lever controlled, 2 port, 2 position, normally open &
  \tikz\pic at (0,0) {dcv={control mechanism=pedal, directional valve=four port three position a and b to t p blocked in centre position, scale=0.5}}; & Solenoid controlled, 4 port, 3 position, A and B to T, P blocked in centre position \\ 
  \hline
    \tikz\pic at (0,0) {dcv={control mechanism=solenoid, directional valve=three port two position normaly open, scale=0.5}}; & Solenoid controlled, 3 port, 2 position, normally open &
  \tikz\pic at (0,0) {dcv={control mechanism=pedal, directional valve=infinite positioning, scale=0.5}}; & Solenoid controlled, infinite positioning \\ 
  \hline
  \tikz\pic at (0,0) {dcv={control mechanism=solenoid, directional valve=three port two position normaly closed, scale=0.5}}; & Solenoid controlled, 3 port, 2 position, normally closed
    & & \\ 
  \hline
  \tikz\pic at (0,0) {dcv={control mechanism=solenoid, directional valve=four port two position normaly open in centre position, scale=0.5}}; & Solenoid controlled, 4 port, 2 position, normally open in center position
    & & \\ 
  \hline
\end{tabular}
  \caption{Directional control valves symbols.}
  \label{fig:directional-control-valves-symbols}
\end{figure}
...
Figure~\ref{fig:hydraulic-pumps-symbols} shows the symbols for hydraulic pumps.
\begin{figure}[H]
  \center
  \begin{tabular}{ | >{\centering\arraybackslash}m{3cm} | m{4cm} | } 
  \hline
  \tikz\pic at (0,0) {fixed displacment pump}; & Fixed displacement pump \\ 
  \hline
  \tikz\pic at (0,0) {variable displacment pump}; & Variable Displacment pump \\ 
  \hline
\end{tabular}
  \caption{Hydraulic pumps symbols.}
  \label{fig:hydraulic-pumps-symbols}
\end{figure}
