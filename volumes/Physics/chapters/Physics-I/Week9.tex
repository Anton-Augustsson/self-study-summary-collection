...
Some equations that are useful are listed below.
...
\begin{tikzpicture}[scale=1]
  ...
  \fill (0.5,0.15) circle (1pt) node[above=3mm] {$P$};
  \fill (1.5,0.15) circle (1pt) node[above=3mm] {$C$};
  \draw[dotted] (0.5,0.15) -- (0.5,-1);
  \draw[dotted] (1.5,0.15) -- (1.5,-1);
  \draw[{Stealth}-{Stealth}] (0.5,-1) -- (1.5,-1) node[midway, above=1mm] {$d$};
  \draw[-{Stealth}] (3.4,0.5) arc[start angle=30, end angle=-45, radius=0.8cm] node[midway, right] {$M\ell$} node[right] {$\omega$};
  \end{scope}
...
\begin{tikzpicture}[scale=1]
  ...
  \draw (-0.2,-2) rectangle (0.2,2);
  \fill (0,0) circle (1pt) node[midway, left=2mm] {$C$};
  \draw[red, -{Stealth}] (-1,1.5) -- (-0.2,1.5) node[midway, above=1mm] {$I$};
  \draw[red, -{Stealth}] (0.2,0) -- (1,0) node[midway, below=1mm] {$v_{cm}$};
  \draw[dotted] (-0.2,1.5) -- (1,1.5);
  \draw[dotted] (-0.2,0) -- (6,0) node[midway, above=16mm] {$M\ell$};
  \draw [decorate, decoration = {brace, amplitude=2mm}] (0.2, 1.5) -- (0.2, 0) node[midway, right=2mm] {$d$};
  \draw[red, -{Stealth}] (0.5,2.2) arc[start angle=90, end angle=0, radius=0.8cm] node[right] {$\omega_{cm}$};

  \draw[rotate around={-45:(4.2,0)}] (4,-2) rectangle (4.4,2);
  \draw[red, -{Stealth}] (5.8,2) arc[start angle=90, end angle=0, radius=0.8cm] node[right] {$\omega_{cm}$};
  \fill (4.2,0) circle (1pt);
  \draw[red, -{Stealth}] (4.2,0) -- (5,0) node[midway, below=1mm] {$v_{cm}$};
...
\begin{tikzpicture}[scale=1]
  ...
  \draw (-0.2,-2) rectangle (0.2,2);
  \fill (0,1) circle (1pt);
  \fill (0,0) circle (1pt) node[left=2mm] {$C$};
  \draw[red, -{Stealth}] (0,1) -- (0,0) node[midway, left=4mm] {$\vec{r}_P$};
  \draw[{Stealth}-{Stealth}] (0.5,1) -- (0.5,0) node[midway, right=2mm] {$\ell$};
  \draw[dotted] (0,1) -- (1,1);
  \draw[dotted] (0,0) -- (1,0);
  \end{scope}

  \pgfmathsetmacro{\Px}{-0.45}
  \coordinate (P) at (\Px,0.9);
  \draw (\Px,-1) arc[start angle=270, end angle=320, radius=0.8cm] node[midway, below] {$\theta$};
  \draw[dotted] (\Px,-2) -- (P) node[left=4mm] {$P$};
  \draw[red, -{Stealth}] (0,0) -- (0,-0.8) node[midway, right=4mm] {$Mg$};
...
\begin{tikzpicture}[scale=1]
  ...
  \pgfmathsetmacro{\diameter}{3.5};
  \pgfmathsetmacro{\radius}{\diameter/2};
  \pgfmathsetmacro{\alpha}{15};
  \pgfmathsetmacro{\radAlpha}{\alpha * pi / 180};
  \pgfmathsetmacro{\Ax}{\radius*cos(\alpha)};
  \pgfmathsetmacro{\Ay}{\radius*sin(\alpha)};
  \pgfmathsetmacro{\Sty}{\diameter/2}; // Start theta y
  \pgfmathsetmacro{\Dpc}{\diameter/2}; // Diameter P C
  \pgfmathsetmacro{\Dc}{\diameter}; // Diameter C
  \pgfmathsetmacro{\Aa}{300}; // Alpha angle
  \coordinate (A) at (\Ax,\Ay);
  \coordinate (C) at (0,0);
  \coordinate (P) at (0,\diameter);

  \draw (C) circle (\diameter cm);
  \fill (C) circle (1pt) node[left] {$C$};
  \fill (P) circle (1pt) node[left=6mm] {$P$};
  \fill (A) circle (1pt);
  \draw[red, -{Stealth}] (P) -- (A) node[midway, right=4mm] {$\vec{r}_P$};
  \draw[red, -{Stealth}] (A) -- (\Ax, -1) node[midway, right=2mm] {$Mg$};

  \draw (0,\Sty) arc[start angle=270, end angle=\Aa, radius=\Dpc cm] node[midway, below] {$\theta$};
  \draw (0,0) arc[start angle=270, end angle=\Aa, radius=\Dc cm];
  \draw[dotted] (P) -- (C);
  \draw[dotted] (C) -- (0,-\diameter) node[midway, right] {R};
...
\begin{tikzpicture}[scale=1]
  ...
  % Draw the pendulum shape
  \path[fill=gray!20, draw=black]
    plot[smooth cycle, tension=1] coordinates {
      (0,0.2) % Top pivot
      (0.4,-1.5) % Right middle
      (0.8,-2.5) % Right lower bulge
      (0,-3.0) % Bottom tip
      (-0.8,-2.5) % Left lower bulge
      (-0.4,-1.5) % Left middle
    };

  % Mark the pivot point
  \fill (0,0) circle (1.5pt);
  \node[above right] at (0,0) {\textbf{S}};

  % Mark center of mass
  \fill (0,-2.2) circle (1.5pt);
  \node[right] at (0,-2.2) {\textbf{cm}};

  % Dashed line connecting S to center of mass
  \draw[dashed] (0,0) -- (0,-2.2) node[midway, left] {\textbf{d}};

  % Gravity vector
  \draw[thick,->] (2,-0.5) -- (2,-1.5) node[midway, right] {\Huge$\vec{\mathbf{g}}$};

  % Axis indicators
  \draw[thick] (-2.2,-3) -- (-1.5,-3) node[right] {\textbf{+x}};
  \draw[thick] (-2.2,-3) -- (-2.2,-2.3) node[above] {\textbf{+y}};

  % Label "Pivot"
  \draw[thick] (-0.5,0) to[out=180, in=90] (-1, -0.5);
  \node[left] at (-1,-0.5) {\textit{Pivot}};
