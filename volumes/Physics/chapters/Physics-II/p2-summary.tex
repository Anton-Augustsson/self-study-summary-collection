...
Electric flux, denoted as $\Phi_E$, is the amount of electric field lines that penetrates a given surface. 
...
\end{tikzpicture}
  \caption{Electric flux penetrating a surface.}
  \label{fig:electirc-flux-penetrating-a-surface}
\end{figure}

Consider the surface shown in Figure~\ref{fig:electirc-flux-penetrating-a-surface}, where $A$ is the area of the surface. Let $\vec{\boldsymbol{A}}=A\hat{\boldsymbol{n}}$ be defined as the area vector, where $\hat{\boldsymbol{n}}$ is the normal direction. The electric field and the normal direction are parallel and perpendicular to the surface. The electric flux is then:
\begin{equation}
...
\end{tikzpicture}
  \caption{Electric flux penetrating a surface at an angle.}
  \label{fig:electric-flux-penetrating-a-surface-at-an-angle}
\end{figure}


\subsection{Gauss's Law}

In spherical coordinates, a small area element on the sphere, shown in Figure~\ref{fig:a-small-area-element-on-a-spherical-surface.}, is given by
\begin{equation}
  d\vec{\boldsymbol{A}} = r^2\sin{\theta}\,d\theta\,d\phi\,\hat{\boldsymbol{r}}
\end{equation}
...
\begin{tikzpicture}
  ...
  % Arc in xy-plane
  \draw[thick] (0,\r,0) arc[start angle=90, end angle=0, radius=\r];
  \begin{scope}[rotate around y=-35]
    \draw[thick] (0,\r,0) arc[start angle=90, end angle=0, radius=\r];
  \end{scope}
  \begin{scope}[rotate around y=-55]
    \draw[thick] (0,\r,0) arc[start angle=90, end angle=0, radius=\r];
  \end{scope}

  \draw[thin] (0,0.4,0) arc[start angle=90, end angle=76, radius=0.4];
  \draw[thin] (0.05,0.4,0) -- (-0.4,1.0,0) node[fill=white] {\small$\theta$};
  \draw[thin] (0.2,0.9,0) arc[start angle=90, end angle=82, radius=0.9];
  \draw[thin] (0.27,0.9,0) -- (0.45,0.4,0) node[below] {\small$d\theta$};
  ...
  \begin{scope}[rotate around x=90]
      \draw[thick] (0,\r,0) arc[start angle=90, end angle=0, radius=\r];
      \draw[thick] (0,{\r*cos(55)},{-\r*sin(55)}) arc[start angle=90, end angle=0, radius={\r*cos(55)}];
      \draw[thick] (0,{\r*cos(45)},{-\r*sin(45)}) arc[start angle=90, end angle=0, radius={\r*cos(45)}];

      \draw[thin] (0,0.6,0) arc[start angle=90, end angle=56, radius=0.6] node[midway, below] {\small$\phi$};
      \draw[thin] (0.7,1.0,0) arc[start angle=90, end angle=5, radius=0.3];
      \draw[thin] (0.9,0.95,0) -- (0.6,2.2,0) node[left] {\small$d\phi$};
  \end{scope}

  \coordinate (s1) at ({\r*cos(50)},{\r*sin(83)},{\r*sin(50)});
  \coordinate (s2) at ({\r*cos(61)},{\r*sin(80)},{\r*sin(61)});
  \coordinate (s3) at ({\r*cos(59)},{\r*sin(55)},{\r*sin(59)});
  \coordinate (s4) at ({\r*cos(45)},{\r*sin(55)},{\r*sin(45)});
  \coordinate (sC) at ($0.25*(s1)+0.25*(s2)+0.25*(s3)+0.25*(s4)$);

  \draw[dashed] (0,0,0) -- (s1);
  \draw (0,0,0) -- (s2);
  \draw (0,0,0) -- (s3);
  \draw (0,0,0) -- (s4);
  \draw[dashed] (0,0,0) -- ({\r*cos(35)},0,{\r*sin(35)});
  \draw[dashed] (0,0,0) -- ({\r*cos(55)},0,{\r*sin(55)});

  \path[draw=blue, fill=blue!20, thick, opacity = 0.8] (s1) -- (s2) -- (s3) -- (s4) -- (s1);

  \filldraw (sC) circle (1pt);
 
\end{tikzpicture}
  \caption{A small area element on a spherical surface of radius $r$.}
  \label{fig:a-small-area-element-on-a-spherical-surface.}
\end{figure}
